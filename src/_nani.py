...
class Number(nani.Number):
    ...
    Attributes
    ----------
    type : ZakPackage numerical type
...
Number.__new__.__defaults__ = (ZakPackage._numeric.Float32, 0, None)
...
class Structure(nani.Structure):
    ...
    Attributes
    ----------
    fields : tuple of ZakPackage.Field or compatible tuple
...
def _consolidate(data_type):
    ...
    Specifically, convert any ZakPackage type into its NumPy equivalent.
    """
    ...
    if isinstance(data_type, Number):
        out = data_type._replace(type=ZakPackage._numeric.to_numpy(data_type.type))
