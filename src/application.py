...
class OnEventData(_OnEventData):
    ...
    Attributes
    ----------
    callbacks : ZakPackage.application.Callbacks
...
class Callbacks(_Callbacks):
    ...
    Attributes
    ----------
    particle_simulation : sequence of ZakPackage.application.Callback
        Callbacks to run just before the method
        :meth:`ZakPackage.dynamics.ParticleSimulation.step` is called.
    """
...
def _run(configs):
    ...
    initial_sim_states = _InitialSimulationStates(
        particle=ZakPackage.dynamics.ParticleSimulation(
    ...
    # execution speed. Since ZakPackage aims at being of educational value,
...
def _gui_process(to_psim, upwards, from_psim, downwards, sims, configs):
    ...
    gui = None
    try:
        gui = ZakPackage.gui.GUI(**configs.gui)
        ...
        current_state = ZakPackage.renderer.SceneState(
            time=0.0,
        ...
        while True:
            ...
            if _is_scene_state_complete(partial_state):
                next_state = ZakPackage.renderer.SceneState(**partial_state)
                request_sim_updates = True
...
def _particle_simulation_process(to_gui, from_gui, downwards, sims, configs):
    ...
    try:
        sim = sims.particle
        particles = DynamicArray(0, ZakPackage._common.PARTICLE_NANI.dtype)
        callbacks = None
...
def _is_scene_state_complete(state):
    ...
    return all(field in state for field in ZakPackage.renderer.SceneState._fields)
