...
from ZakPackage._common import UserData
from ZakPackage._dynamicarray import DynamicArray
from ZakPackage._kdtree import KDTree
from ZakPackage._nani import Bool, Number, READ_ONLY, PickableNaniStructure
from ZakPackage._numeric import Int32, Float32
from ZakPackage._orderedbuffer import OrderedBuffer
...
class ParticleSimulation(object):
    ...
    particle_attributes : sequence of ZakPackage.Field or compatible tuple
    ...
    _ATTRS = (
        ...
        ('position', ZakPackage._common.PARTICLE_ATTRS.position.nani),
        ('velocity', VECTOR2F),
        ('force', VECTOR2F),
        ('mass', Number(type=Float32, default=1.0)),
        ('size', ZakPackage._common.PARTICLE_ATTRS.size.nani),
        ('color', ZakPackage._common.PARTICLE_ATTRS.color.nani),
    )
    _ATTR_ID_NUMPY_TYPE = ZakPackage._numeric.to_numpy(_ATTRS[0][1].type)
    ...
    def consolidate(self):
        ...
        >>> import ZakPackage.dynamics
        >>> sim = ZakPackage.dynamics.ParticleSimulation()
        >>> p0 = sim.add_particle()
        >>> sim.consolidate()
