...
def get_types():
    """Retrieve all the numeric types supported by ZakPackage."""
    return _TYPES


def get_type_size(this_type):
    """Retrieve the size in bytes of a ZakPackage type."""
    return _TYPES_DATA[this_type]['size']


def get_type_literal(this_type):
    """Retrieve the string literal of a ZakPackage type."""
    return _TYPES_DATA[this_type]['literal']


def from_ctype(ctype_type):
    """Convert a C type to its ZakPackage equivalent."""
    return _CTYPE_TO_THIS[ctype_type]


def from_gl(gl_type):
    """Convert an OpenGL type to its ZakPackage equivalent."""
    return _GL_TO_THIS[gl_type]


def from_numpy(numpy_type):
    """Convert a NumPy type to its ZakPackage equivalent."""
    return _NUMPY_TO_THIS[numpy_type]


def to_ctype(this_type):
    """Convert a ZakPackage type to its ctypes equivalent."""
    return _TYPES_DATA[this_type]['ctype']


def to_gl(this_type):
    """Convert a ZakPackage type to its OpenGL equivalent."""
    return _TYPES_DATA[this_type]['gl']


def to_numpy(this_type):
    """Convert a ZakPackage type to its NumPy equivalent."""
