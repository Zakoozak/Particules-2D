...
import ZakPackage._common
import ZakPackage._numeric
from ZakPackage._common import GraphicsAPI, GLProfile, ParticleDisplay
from ZakPackage._vectors import Vector2f
        ...
        'attributes': (
            {
                'name': 'position',
                'location': 0,
                'count': ZakPackage._common.PARTICLE_ATTRS.position.count,
                'type': ZakPackage._numeric.to_gl(
                    ZakPackage._common.PARTICLE_ATTRS.position.element_type),
            ...
            },
            {
                'name': 'size',
                'location': 1,
                'count': ZakPackage._common.PARTICLE_ATTRS.size.count,
                'type': ZakPackage._numeric.to_gl(
                    ZakPackage._common.PARTICLE_ATTRS.size.element_type),
                'normalized': gl.GL_FALSE,
                'divisor': 1,
            },
            {
                'name': 'color',
                'location': 2,
                'count': ZakPackage._common.PARTICLE_ATTRS.color.count,
                'type': ZakPackage._numeric.to_gl(
                    ZakPackage._common.PARTICLE_ATTRS.color.element_type),
                'normalized': gl.GL_FALSE,
                'divisor': 1,
            },
        ),
...
class State(_State):
    ...
    background_color : ZakPackage.Vector4f
        Color for the background.
    grid_color : ZakPackage.Vector4f
        Color for the grid.
    grid_origin_color : ZakPackage.Vector4f
...
class Renderer(object):
    ...
    def render(self, state, scene_state):
        ...
        Parameters
        ----------
        state : ZakPackage.renderer.State
            Renderer state.
        scene_state : ZakPackage.renderer.SceneState
            Scene state.
        """
...
def _get_vertex_formats(vertex_formats_data, bufs):
    ...
    def get_gl_type_size(gl_type):
        return ZakPackage._numeric.get_type_size(ZakPackage._numeric.from_gl(gl_type))

    def gl_to_numpy_type(gl_type):
        return ZakPackage._numeric.to_numpy(ZakPackage._numeric.from_gl(gl_type))
